/**
 * jspsych-audio-button-response2 (DISABLED BUTTONS)
 * Kristin Diep
 *
...
jsPsych.plugins["audio-button-response2"] = (function () {
  var plugin = {};

  jsPsych.pluginAPI.registerPreload('audio-button-response2', 'stimulus', 'audio');

  plugin.info = {
    name: 'audio-button-response2',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function (display_element, trial) {
    ...
    function setupTrial() {
      ...
      for (var i = 0; i < trial.choices.length; i++) {
        ...
        if (i < 3){
          html += '<div class="jspsych-audio-button-response-button" style=" pointer-events: none;cursor: not-allowed;display: inline-block; margin:' + trial.margin_vertical + ' ' + trial.margin_horizontal + '" id="jspsych-audio-button-response-button-' + i + '" data-choice="' + i + '">' + str + '</div>';
        }else{
          html += '<div class="jspsych-audio-button-response-button" style="display: inline-block; margin:' + trial.margin_vertical + ' ' + trial.margin_horizontal + '" id="jspsych-audio-button-response-button-' + i + '" data-choice="' + i + '">' + str + '</div>';
        }
      }
      html += '</div>';
